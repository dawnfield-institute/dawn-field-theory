...
## Independent epistemic audit

[`ORRERY_AUDIT.md`](ORRERY_AUDIT.md) is a mechanical audit of the claim graph behind this release, generated by [Orrery](https://github.com/dawnfield-institute/orrery) over `foundational/experiments/`. It keeps the two axes separate — **status** (the human verdict: validated · open · falsified) and **instrumentation** (how legibly the corpus evidences each claim, *measured*) — and surfaces contradictions, citation drift, and coverage deterministically. It is an outside-tool check on the same claims the papers make: the precision constants (α, Koide, sin²θ_W) surface validated but *sparsely instrumented*; the structural results (ADE→Lorentz, the Born rule, D₄) surface well-instrumented; the falsified predictions surface falsified. Regenerate with `orrery report`.
