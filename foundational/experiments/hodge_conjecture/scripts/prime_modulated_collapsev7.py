...
with open("peak_symmetry_data.csv", "w", newline="") as csvfile:
    ...
    writer.writerow(["Index", "Modulation", "Peak Count", "FFT Symmetry Score", "Peak Positions"])
    for n, p in zip(modulations, primes):
        peak_count = peak_counts[n][0]
        score = symmetry_scores[n]
        positions = ";".join(map(str, peak_positions[n]))
        writer.writerow([p, n, peak_count, score, positions])
