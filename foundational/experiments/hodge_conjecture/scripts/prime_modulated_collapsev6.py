...
def run_simulation(n, grid_size=256, steps=100, tau_c=0.65, gamma_energy=0.95, gamma_symbolic=0.96, lambda_reinforce=0.05):
    ...
    return symbolic_history[-1], symbolic_history  # Return final field and full history
...
for p, n in zip(primes, modulations):
    final_field, history = run_simulation(n)
    final_fields[n] = final_field
    ...
    peak_positions[n] = positions

    fft_map, score = compute_fft_symmetry(density)
    symmetry_maps[n] = fft_map
    symmetry_scores[n] = score

    mod_p_fields[n] = apply_mod_p_transform(final_field, p)

# Export peak and symmetry data to CSV
with open("peak_symmetry_data.csv", "w", newline="") as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(["Prime", "Modulation", "Peak Count", "FFT Symmetry Score", "Peak Positions"])
    for n in modulations:
        prime = round(n / pi)
        peak_count = peak_counts[n][0]
        score = symmetry_scores[n]
        positions = ";".join(map(str, peak_positions[n]))
        writer.writerow([prime, n, peak_count, score, positions])

# Plot symmetry score vs prime
plt.figure(figsize=(10, 6))
primes_list = [round(n / pi) for n in modulations]
scores_list = [symmetry_scores[n] for n in modulations]
plt.plot(primes_list, scores_list, marker='o')
plt.title("FFT Symmetry Score vs Prime Index")
plt.xlabel("Prime Index")
plt.ylabel("FFT Symmetry Score")
plt.grid(True)
plt.savefig("fft_symmetry_vs_prime.png")
plt.show()
...
for i, (n, density) in enumerate(density_maps.items()):
    ...
    ax_mod.set_title(f"Symbolic Field mod {prime}")
    ax_mod.axis('off')
    fig.colorbar(im_mod, ax=ax_mod, fraction=0.046, pad=0.04)

plt.suptitle("Prime-Modulated Symbolic Collapse: Full Analysis Suite", fontsize=20)
plt.tight_layout()
plt.savefig("prime_modulated_full_analysis.png")
plt.show()
