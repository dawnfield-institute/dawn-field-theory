...
prime_scores = [s for p in [2, 3, 5, 7, 11] for s in all_scores[p]]
nonprime_scores = [s for p in [4, 6, 8, 9, 10] for s in all_scores[p]]
prime_peaks = [s for p in [2, 3, 5, 7, 11] for s in all_peaks[p]]
nonprime_peaks = [s for p in [4, 6, 8, 9, 10] for s in all_peaks[p]]
...
print("Prime Mean Peaks:", mean(prime_peaks), "+/-", stdev(prime_peaks))
print("Non-Prime Mean Peaks:", mean(nonprime_peaks), "+/-", stdev(nonprime_peaks))

print("Symmetry t-test:", ttest_ind(prime_scores, nonprime_scores))
print("Peaks t-test:", ttest_ind(prime_peaks, nonprime_peaks))

# Visualization: Symmetry score distribution
sns.boxplot(x=["Prime"]*len(prime_scores) + ["Non-Prime"]*len(nonprime_scores),
            y=prime_scores + nonprime_scores)
plt.title("FFT Symmetry Score Distribution: Prime vs Non-Prime")
plt.savefig("symmetry_score_boxplot.png")
plt.show()

# Visualization: Peak count distribution
sns.boxplot(x=["Prime"]*len(prime_peaks) + ["Non-Prime"]*len(nonprime_peaks),
            y=prime_peaks + nonprime_peaks)
plt.title("Peak Count Distribution: Prime vs Non-Prime")
plt.savefig("peak_count_boxplot.png")
plt.show()
