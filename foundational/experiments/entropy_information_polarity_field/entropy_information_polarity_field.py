...
def initialize_field(resolution, seed=314159, device="cuda", mode="blackhole"):
                ...
                for dz in range(-r, r + 1):
                    if dx**2 + dy**2 + dz**2 <= r**2:
                        ...
                        if 0 <= x < resolution and 0 <= y < resolution and 0 <= z < resolution:
                            ...
                            symbolic_field[x, y, z] = 0.9  # Boosted from 0.1 to 0.9
                            ancestry_field[x, y, z] = (x * resolution + y + z) % (resolution ** 2)
    ...
    asymmetry = asymmetry.to(entropy_field.device)  # Ensure device match
    entropy_field[:, :, :] += asymmetry.unsqueeze(-1)

    # Add azimuthal twist to entropy to break radial symmetry
    # Inject vortex-shaped entropy asymmetry (add entropy field twist with radial decay)
    # Ensure all tensors are on the correct device
    X, Y, Z = torch.meshgrid(
        torch.arange(resolution, device=entropy_field.device),
        torch.arange(resolution, device=entropy_field.device),
        torch.arange(resolution, device=entropy_field.device),
        indexing='ij'
    )
    cx = resolution // 2
    r = torch.sqrt((X - cx)**2 + (Y - cx)**2 + 1e-5)
    theta = torch.atan2(Y - cx, X - cx)
    entropy_field += 0.1 * torch.sin(theta * 3) * torch.exp(-((r / 10)**2))

    # Inject rotational asymmetry into symbolic field
    X, Y, Z = torch.meshgrid(
        torch.arange(resolution, device=symbolic_field.device),
        torch.arange(resolution, device=symbolic_field.device),
        torch.arange(resolution, device=symbolic_field.device),
        indexing='ij'
    )
    ...
    r = torch.sqrt((X - cx)**2 + (Y - cx)**2 + 1e-5)
    theta = torch.atan2(Y - cx, X - cx)
    symbolic_field += 0.1 * torch.sin(theta * 3) * torch.exp(-((r / 10)**2))
...
def collapse_decision(entropy_field, symbolic_field, lineage_trace, curl=None):
    ...
    # Modulate collapse by local curl if provided
    if curl is not None:
        curl_factor = 1 + curl.abs().clamp(0, 1)
        recursive_gate = recursive_gate * curl_factor
    collapse = (entropy_field * recursive_gate) > 0.5
    # Curl-driven collapse feedback
    if curl is not None:
        dynamic_gate = torch.sigmoid(curl.clamp(-1, 1))  # Normalize torque effect
        collapse = collapse & (dynamic_gate > 0.5)       # Torque must support collapse
...
def compute_symbolic_recursion(symbolic_field, recursion_memory):
    ...
    decay = 0.98  # Memory decay factor
    updated_memory = decay * recursion_memory + symbolic_field
    return diffused.squeeze(), updated_memory
...
def simulate_step(entropy_field, symbolic_field, lineage_trace, recursion_memory, ancestry_field, threshold, mode, curl_memory=None):
    ...
    entropy_grad, directional_grads = compute_entropy_gradient(entropy_field)
    symbolic_force, recursion_memory = compute_symbolic_recursion(symbolic_field, recursion_memory)

    # Apply symbolic mutation in active lineage regions to prevent saturation
    mutation_rate = 0.02
    lineage_mask = (lineage_trace > 0).float()
    symbolic_field += (torch.rand_like(symbolic_field) - 0.5) * mutation_rate * lineage_mask
    ...
    # Compute curl from the interplay of entropy and symbolic field gradients
    _, entropy_grads = compute_entropy_gradient(entropy_field)
    _, symbolic_grads = compute_entropy_gradient(symbolic_field)

    # Vector field from entropy and symbolic differential interplay
    vector_field = tuple(e * s for e, s in zip(entropy_grads, symbolic_grads))
    curl = compute_curl(vector_field)
    # Optional: print curl shape for debugging
    # print(f"Curl shape: {curl.shape}")

    # Track curl memory accumulation
    if 'curl_memory' not in simulate_step.__dict__:
        simulate_step.curl_memory = torch.zeros_like(curl)
    simulate_step.curl_memory += curl.abs()
    ...
    # Print Curl Metrics
    print(f"Step: Curl mean={curl.mean().item():.6f}, max={curl.max().item():.6f}")

    entropy_grad = entropy_grad + (symbolic_field > 0.9).float() * -0.3
    collapse, field_potential = collapse_decision(entropy_grad, symbolic_force, lineage_trace, curl)
    # 🧩 Add:
    ...
    if mode == "blackhole":
        ...
        entropy_field = torch.clamp(entropy_field + collapse * 0.1, max=1.0)
    elif mode == "whitehole":
        symbolic_field = symbolic_field + collapse * 0.2
        entropy_field = entropy_field - collapse * 0.2
    lineage_trace += collapse

    # Symbolic mutation driven by active lineage regions
    mutation_rate = 0.02  # Adjust as needed
    lineage_mask = (lineage_trace > 0).float()
    symbolic_field += (torch.rand_like(symbolic_field) - 0.5) * mutation_rate * lineage_mask
    ...
    ancestry_field[inherit_mask] = ((ancestry_field[inherit_mask].float() + neighbor_sum[inherit_mask]).round() % (resolution ** 2)).int()
    ...
    ancestry_field = ancestry_field.int()
    ...
    return entropy_field.clamp(0, 1), symbolic_field.clamp(0, 1), lineage_trace, recursion_memory, ancestry_field, curl, directional_grads, collapse
...
def run_simulation(steps=100, resolution=64, threshold=0.1, device="cuda", mode="blackhole"):
    ...
    previous_curl = torch.zeros_like(entropy_field)
    for step in range(steps):
        ...
        print(f"Step {step}: Curl mean={curl.mean().item():.6f}, max={curl.max().item():.6f}")
        print(f"Step {step}: Curl Δ = {torch.norm(curl - previous_curl).item():.6f}")
        previous_curl = curl.clone()
