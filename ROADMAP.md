...
| prime_gap_ledger | Founded 2026-09-07 (exp_01 sealed `9213386c`, 0/3 by its rules). Recorded: the window's consecutive-residue bias depends on the sieve depth alone and equals the primorial loop's exact bias; at the primes the loop over-predicts by a scale-free residual ε(u) ≈ 0.0095 at u = 2 (10⁶–10⁹). **Round 2 (exp_02, seal ae67f522) 3/4:** the delta is the position of the read — the residual is a scale-free function of the arc's depth, is one tenth of Buchstab's measured density deficit with the sign, and changes sign where ω(u) > e^{−γ} (CONFIRM ×3; the proportionality INCONCLUSIVE by a clause conflict). **Round 3 (exp_03, seal ff63f8d5) 2/2:** the shell modulation was the uniform loop's own depth profile; the primes' arc is the loop read at the density-matched depth y^{1/ratio} (Buchstab's ratio), and the departure ρ from that read is zero to ±0.001 at 10⁹–10¹⁰ at every modulus and through the flip — the delta is the position, and nothing else. Open (not opened): the depth-shift law at moduli with a prime factor above y; a closed form for the loop's β_q(y); the lattice shape metric — [`experiments/studies/prime_gap_ledger/`](experiments/studies/prime_gap_ledger/) |
