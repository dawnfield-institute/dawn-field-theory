# CIMM vCPU â€” Expanded with Sensor Mapping, Reinforcement, and Logic Gate Training
...
for idx, score in enumerate(io_scores):
    print(f"Input Bit {io_input[idx]} â†’ Avg Collapse at Target: {score:.2f}")
...
def visualize_qpl_deltas(snapshots, titles):
    ...
    for i in range(1, len(snapshots)):
        ...
        axes[i - 1].set_title(f"Î” QPL: {titles[i - 1]} â†’ {titles[i]}")
        axes[i - 1].axis('off')
    plt.tight_layout()
    plt.show()


# Phase 4: Logic Gate Task â€” AND, OR, XOR (Planned)
