...
HYPOTHESIS: The Pythia phi-crossing finding (p=0.0014) should replicate in 
...
- Test if layer-wise dynamics show phi-related structure
...
def compute_attention_statistics(model, tokenizer, texts: list) -> dict:
    ...
    - phi_crossings: layers where ratio crosses phi or 1/phi
    """
    all_layer_entropies = []
    ...
    # Find phi-crossings (where ratio crosses phi or 1/phi)
    phi_crossings = []
...
def analyze_weight_structure(model) -> dict:
    ...
    - phi-related scaling between layers
    """
    layer_norms = []
...
def compare_to_pythia(gpt2_result: dict, pythia_reference: dict = None) -> dict:
    ...
    - Late training ratio: ~2.2 (between phi and 2.0)
    ...
    if pythia_reference is None:
        ...
        pythia_reference = {
            ...
            'late_ratio_std': 0.15,
            'phi_distance': 0.69,  # Distance from phi=1.618
            'convergence': True
        }
...
def main():
    ...
    print("="*60)
    print()
    print("HYPOTHESIS: Pythia phi-crossing should replicate in GPT-2")
    ...
    for model_name in models:
        try:
            ...
            print(f"  phi-crossings: {len(result['attention_stats']['phi_crossings'])}")
    ...
    if len(all_ratios) > 5:
        ...
        # Test distance from phi and 1/phi
        ...
        print(f"Mean distance from phi/1/phi: {mean_phi_distance:.4f}")
    ...
    if similar_count == len(all_results):
        ...
        print("   phi-related dynamics are ARCHITECTURE-INDEPENDENT")
        confirmation = True
    ...
    output = {
        ...
        'hypothesis': 'Pythia phi-crossing replicates in GPT-2',
