#!/usr/bin/env python3
"""Panel L1b (EXPLORING): are the accidental golden trees SYMMETRY-QUOTIENT folds?
...
def orbits(G):
    n=G.number_of_nodes(); orb={v:{v} for v in G}
    for m in GraphMatcher(G,G).isomorphisms_iter():
        for v in G: orb[v].add(m[v])
    seen=set(); out=[]
    for v in sorted(G):
        key=frozenset(orb[v])
        if key not in seen: seen.add(key); out.append(sorted(key))
    return out
