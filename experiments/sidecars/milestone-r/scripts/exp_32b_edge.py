#!/usr/bin/env python3
"""exp_32b — the edge as a number, re-sealed: score the fine sweep, the gravity arms and the size arm
against the SEALED registration.

    python exp_32b_edge.py results/exp_32b_edge_grid_fine_<ts>.json results/exp_32b_edge_grid_g_<ts>.json results/exp_32b_edge_grid_double_<ts>.json
    python exp_32b_edge.py --selftest

Three grids from reality-engine POC-12 exp_04_aggregate.py (fine: results/full_r2b_fine, g arms:
results/full_r2b_g, size arm: results/full_r2b_n8000), copied into results/. REFUSES seeds outside the
...
(journals/2026-09-16_exp32b_registration.md §1/§4/§7).
"""
...
JOURNAL = HERE.parent / "journals" / "2026-09-16_exp32b_registration.md"

# ---- the sealed objects and thresholds, verbatim ------------------------------------------------
SEEDS = (16, 17, 18)                                   # fresh; anything else VOIDS the grid
FINE = (1.0, 1.05, 1.1, 1.15, 1.2, 1.25, 1.3)          # the fine sweep (base geometry, g = 1.5)
G_ARMS = (0.75, 3.0)                                   # the gravity arms, at kappa in {1.00, 1.25}
EDGE_KAPPAS = (1.0, 1.25)
BASE_G = 1.5
SIZE_ARM = "double"                                    # n = 8000, box 75.6
SIZE_KAPPAS = (1.0, 1.25, 1.3)                         # 1.30 is REPORTED, not scored (§1)
T2_FRAC_BAND = (0.40, 0.60)                            # T2: positive-work fraction at the crossing step (§0.4)
...
AT_CAP_MAX = 0.05
IDENTITY_TOL = 0.08                                    # |dE_SEC − (T − W_p)| <= 0.08 |U0|  (§0.7: measured 0.0154-0.0535 over 36 runs)
...
SELFTEST_STRINGS = [
    "exactly once\nacross the swept range κ ∈ [1.00, 1.30]**, from negative to positive",
    "**No spread bar** (§0.2)",
    "same bracketing step as the total, or the adjacent one",
    "within **[0.40, 0.60]** (§0.4)",
    "g = 0.75, g = 3.0",
    "W_p < 0 at κ = 1.00 and W_p > 0 at κ = 1.25",
    "n = 8000, 3/3 seeds",
    "within [0.45, 0.60]",
    "**|ΔE_SEC − (T − W_p)| ≤ 0.08 |U₀|**",
    "`closure_pac_max` ≤ 0.05; `at_cap_max` ≤ 0.05",
    "W_p within ±0.02 |U₀| of zero at every κ",
    "`budget_bound_frac_max` < 0.01",
    "Seeds:** {16, 17, 18}",
    "**linear\n  interpolation between the bracketing marks**",
    "κ ∈ {1.00, 1.05, 1.10, 1.15, 1.20, 1.25, **1.30**}",
]
...
def check(grids):
    ...
    for key, g in grids.items():
        ...
        # the completeness tuple carries the SEED: without it a grid missing one seed at one kappa
        # still matches, is declared complete, and dies inside score() with a KeyError (§0.10).
        want = ({(s, k, BASE_G, "full") for k in FINE for s in SEEDS} if key == "fine" else
                {(s, k, gg, "full") for k in EDGE_KAPPAS for gg in G_ARMS for s in SEEDS} if key == "g" else
                {(s, k, BASE_G, SIZE_ARM) for k in SIZE_KAPPAS for s in SEEDS})
        have = {(r["seed"], r["kappa"], round(r["config"]["g"], 3), r["size"]) for r in runs}
        if have != want:
...
def score(grids):
    ...
    brackets, crossings, medbr, steps, medsteps = {}, {}, {}, {}, {}
    unresolved = True
    for s in SEEDS:
        ...
        # kappa_c by LINEAR INTERPOLATION between the bracketing marks (§1), not the midpoint
        if len(changes) == 1 and not signs[0] and signs[-1]:
            i = changes[0]; k0, k1, w0, w1 = FINE[i], FINE[i + 1], w[i], w[i + 1]
            brackets[s] = k0 + (k1 - k0) * (-w0) / (w1 - w0)
            steps[s] = i
        else:
            brackets[s] = None; steps[s] = None
        ...
        medsteps[s] = mch[0] if len(mch) == 1 else None
    ...
    # T1: exactly one crossing, negative to positive, across the whole swept range. NO SPREAD BAR (§0.2).
    t1_ok = all(crossings[s] == 1 and brackets[s] is not None for s in SEEDS)
    have_all = all(k is not None for k in kcs)
    res["T1"] = dict(crossings=crossings, kappa_c_per_seed=brackets,
                     kappa_c=(float(np.mean(kcs)) if have_all else None),
                     kappa_c_spread=(float(max(kcs) - min(kcs)) if have_all else None),
                     kappa_c_sd=(float(np.std(kcs, ddof=1)) if have_all else None), ok=bool(t1_ok))
    # T2: the median crosses in the same or adjacent step; the fraction at the crossing step within band
    t2 = []
    for s in SEEDS:
        if steps[s] is None: t2.append(False); continue
        # compare STEP INDICES, never float kappas — same step or the adjacent one
        same = medsteps[s] is not None and abs(steps[s] - medsteps[s]) <= 1
        i = steps[s]
    ...
    res["T4"] = dict(wp_k1=[wp_u0(arm(dbl, 1.0, BASE_G, SIZE_ARM)[s]) for s in SEEDS],
                     wp_k125=[wp_u0(arm(dbl, 1.25, BASE_G, SIZE_ARM)[s]) for s in SEEDS],
                     wp_k130_reported=[wp_u0(arm(dbl, 1.3, BASE_G, SIZE_ARM)[s]) for s in SEEDS], frac_k125=fr,
...
def main(argv):
    ...
    ts = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S"); dest = RESULTS / f"exp_32b_edge_{ts}.json"
    dest.write_text(json.dumps(out, indent=2, default=float), encoding="utf-8")
    r = out["detail"]
    print(f"exp_32b — the edge as a number, re-sealed; seeds {SEEDS}; commits {out['reality_engine_commits']}")
    for s in SEEDS:
    ...
    t = r["T1"]; print(f"  T1 crossings {t['crossings']} kappa_c per seed "
                       f"{ {k: (round(v, 4) if v else v) for k, v in t['kappa_c_per_seed'].items()} } -> "
                       f"kappa_c = {round(t['kappa_c'], 4) if t['kappa_c'] else None} "
                       f"spread {round(t['kappa_c_spread'], 4) if t['kappa_c_spread'] else None} "
                       f"(no bar) -> {out['tests']['T1']}")
