...
| 28 | Dynamical Severance (reality-engine v4 substrate) | E | **registered, not run** | **Sealed 2026-09-05** (`journals/2026-09-05_exp28_registration.md`). First DYNAMICAL run of the derived trigger (exp_15 all-edges overstress at the lattice spacing) and derived form (exp_01 decoupling) as whole-particle severance on reality-engine's v4 particle substrate, which since 2026-09-05 integrates its force law and conserves momentum. M = 4 on the retained set at matched count: T1 the sink is real; T2 holding (percolation vs B0, 3/3 seeds, > 2x pooled std); T3 vs matched-energy drag (K1: any dissipation would do); T4 vs matched-count random severance (K2: removal, not selection). tau in {5, 10, 20} fixed from firing rates alone (pre-declared clause moved the set off tau = 1, which cascades to 92%). Scored by `scripts/exp_28_dynamical_severance.py` from the reality-engine POC-11 grid JSON; thresholds byte-equal to the seal. |
