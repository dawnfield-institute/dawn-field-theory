# Archive — Non-Experiment Material
...
| `devkit-designs/` | `devkit/` | 2025-era architecture design documents — aletheia, brainstem, prometheus, field_decomposition, mcp_server. Despite the name it was never repository tooling (that lives in `tools/`); it is design thinking from the CIMM/QBE period. |
...
None of these is maintained. Both are readable as a record of how the project was thinking
before the milestone stack existed — which is the same reason the experiment archive
exists. See [`STANDARDS.md`](../STANDARDS.md) §2.3 for the era definitions.

**Not archived, deliberately.** `preprints/PACSeries/v0.1` declares itself superseded by v0.2 and would otherwise belong here, but it carries its own Zenodo DOI (10.5281/zenodo.17295103). A published package is a frozen artifact: the snapshot is what makes the DOI reproducible, so it stays where a reader following the DOI expects to find it.
