...
| The prime gap ledger: the consecutive-prime residue bias at every modulus, shells included, equals the primorial loop's bias read at the density-matched depth y^{1/ratio} (ratio = Buchstab's local-to-global density ratio) — the delta between the primes and the loop is the position of the read, and nothing else | prime_gap_ledger exp_01–exp_03 (2026-09-07) | **measured, registered** — exp_03 (seal ff63f8d5) 2/2: ρ = 0 at the primes' arc and along the curve through the flip, 35 cells at 10⁹–10¹⁰ within ±0.001 of r, the flip's sign derived with no free parameter; log y_eff/log y = 1/ratio to four decimals in every cell. exp_02 (ae67f522) 3/4 had found the position reading, the sign flip (3–6σ) and a shell modulation, the last now shown to be the loop's own depth profile β_q (a known answer). exp_01 0/3 by its rules; the exact loop rationals recorded. Andy Farmer's ultrametric tranche beyond position: none resolved |
...
| **The signed cycle: the twist selects the odd exponents and doubles the conductor; twisted strict iff d = 2 and 4 \| n; H_tw = ε·S·H_bal·S⁻¹ on M15's instrument (C₆ = −I becomes I)** | **proven** (2026-09-07) | [theorems](formal/theorems/README.md#the-signed-cycle-the-twist-selects-the-odd-exponents-and-doubles-the-conductor) | [M18 Block F gates](experiments/milestones/milestone18/) (392 grade cells, 74 holonomy cells, M15's 38 K3 rows) | — |
...
| Strict trees exist only at n ≡ 0 (mod 4) (parity law) | **conjecture** | [conjectures](formal/conjectures/m18_open.md) | [M18 exp_19](experiments/milestones/milestone18/) (exhaustive at every n ≤ 24; 257 strict at n = 24, 0 at 6/10/14/18/22 and every odd n); **signed unicyclic graphs n ≤ 14, both classes (exp_20, 2026-09-07): strict only at 8 and 12, and the obstruction is not a rational root** | — |
...
| | Thesis | Score | Key results |
|---|---|---|---|
| **R** [Radiation as ledger severance](experiments/sidecars/milestone-r/) | Radiation is PAC ledger severance | **71/128** | Geiger-Nuttall shown to be a universality theorem; graph Green's function reproduces the hydrogen spectrum to 0.68%; energy scale fixed by alpha(d)^2 m_mediator; **exp_28 (2026-09-05, 0/4): the all-edges overstress trigger, run dynamically on reality-engine's v4 substrate, fires at the extrema of the entropy field — the collapse cores — and removes structure at the onset (T2 −2.6 σ at proxy τ = 20; a null at n = 4000 where half is removed; never above a random subset); C4.1 option 3 closed**; **exp_29 (2026-09-06, 3/4 at both sizes): a PAC ledger on particles makes the substrate bound and conserving and the web survives (4.8–5.7 σ over the unbounded engine); against gravity alone the registered κ = 0.5 arm fails the 2 σ bar (0.2 σ proxy, 1.1 σ at n = 4000) — kill fired; post hoc at n = 4000, κ = 1 beats gravity 3/3 at 2.8 σ; the ledger stays, R1b registers κ = 1 on fresh seeds**; **exp_30 (2026-09-14, 3/4): R1b on fresh seeds with the box-to-range ratio declared — the κ = 1 margin over gravity alone falls from 2.8 σ to 0.8 σ (2/3 seedwise; the control's seeds moved, the arm reproduced), the kill fires and the mapping is retired as a structure-adder; bound at κ = 1 and at the predicted κ = 1.5; the web goes where the bounded pressure's net work turns positive (between κ = 1 and 1.5); R2 runs on the bound substrate**; **exp_31 (2026-09-14, 3/4): the question re-asked paired on seed at matched occupancy on six fresh seeds — the ledgered engine at κ = 0.5 adds connectivity to gravity's web (+0.065 ± 0.015, 6/6, paired t 4.4) without changing what the web is; κ = 1's fattening signature and the edge between κ = 1 and 1.25 (net pressure work changing sign) reproduce 6/6; R2's substrate is κ = 0.5**; **exp_32 (2026-09-15, UNSCORED — the sealed identity gate was set below the integrator's truncation): unofficially the edge is a single sign change on every fresh seed at κ_c ≈ 1.17 (spread ~0.1) and does not move with gravity 12/12; the pair coupling cancels exactly (spec R4′)** |
