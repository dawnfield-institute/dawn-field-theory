...
| The prime gap ledger: the consecutive-prime residue bias equals the primorial loop's exact bias at the truncation depth minus a residual that is one tenth of Buchstab's density deficit, sign included — the loop under-predicts where ω(u) > e^{−γ} (the delta is the position of the read) | prime_gap_ledger exp_01/exp_02 (2026-09-07) | **measured, registered** — exp_02 (seal ae67f522) 3/4: the position reading CONFIRM (R1), the sign flip CONFIRM at 3–6σ in four of six cells with the sign tracking the measured deficit cell by cell (R2), shell independence CONFIRM with a 3σ deeper-shell modulation recorded (R4); the proportionality c ≈ 0.10 INCONCLUSIVE by the seal's own clause conflict (R3). exp_01 scored 0/3 by its rules (converged quantities, a lattice-metric artifact) and recorded the y-only collapse and the exact loop rationals |
